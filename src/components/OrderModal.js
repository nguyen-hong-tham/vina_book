...
export default function OrderModal({
  ...
  onConfirm = () => { }, // hàm gọi khi xác nhận thay đổi trạng thái, nhận ào orderId à newStatus
  ...
  return (
    <di className="fixed inset-0 bg-black/25 backdrop-blur-sm flex items-center justify-center z-50">
      <di className="text-black bg-white rounded-lg shadow-xl p-6 w-96 max-w-full">
        ...
        <di className="flex gap-3">
          ...
          {nextStatuses.length > 0 && (
            <button
              ...
              className={`flex-1 px-4 py-2 text-white rounded transition disabled:opacity-50 ${statusColors[selectedStatus] || 'bg-blue-500 hoer:bg-blue-600'
                }`}
            >
